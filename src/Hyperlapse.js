...
var Hyperlapse = function(container, params) {
	 ...
	 */
    self.onFrame = function(e){};

	/**
	 * @event Hyperlapse#onPlay
	 */
    self.onPlay = function(e){};
     ...
     */
    self.onLoadCanceled = function(e){};

    /**
     * @event Hyperlapse#onLoadProgress
     */
    self.onLoadProgress = function(e){};
	...
	_loader.onPanoramaLoad = function() {
		...
		if (++_point_index != _h_points.length) {
			...
			if (!_cancel_load) {
				_loader.composePanorama(_h_points[_point_index].id);
			} else {
				handleLoadCanceled({});
			}
		} else {
     ...
     */
    self.onLoadComplete = function(e){};
	 ...
	 * @param {HyperlapsePoint} e.point
	 */
    self.onRouteProgress = function(e){};
     ...
     */
    self.onRouteComplete = function(e){};
	...
	var parsePoints = function(response) {
		...
		_loader.find(_raw_points[_point_index], function(err, resp) {

			if (resp.id != _prev_pano_id) {
				_prev_pano_id = resp.id;
/*
				var hp = new HyperlapsePoint(resp.location, resp.id, {
					heading: resp.rotation, //TODO this should probably be resp.heading and then converted to radians
					pitch: resp.pitch,
					elevation: resp.elevation,
					copyright: resp.copyright,
					image_date: resp.imageDate
				});
*/
				_h_points.push(resp);
    ...
    //handle a route from googles directions service
